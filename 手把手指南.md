...
```
2. 把menu_cart.html放進 partials 資料夾  
...
```
         <div
                        ...
                        hx-swap="innerHTML"
                    >
                        {% include 'cart/partials/menu_cart.html' %} //改變path
                    </div>
```
3. cart/views.py
```
def add_to_cart(request, product_id):
    ...
    return render(request, 'cart/partials/menu_cart.html') //改變path
    ...
...
```
...    class Meta:
        verbose_name_plural = 'Categories' //改變Categories
        ordering = ('name',)
...
```
7. product/templates/product/partials/product_list_item.html
```
...
    <a href="{% url 'product' product.slug %}">
        <img src="{{ product.get_thumbnail }}"  class="w-full rounded-xl hover:shadow-lg"> //加了w-full
    </a>
...
```
8. cart/templates/cart/cart.html
```
    <div class="summary w-full md:w-1/4 p-6 bg-gray-100 rounded-xl mt-3"> //加了md:
        <h2 class="uppercase text-lg mb-5">Summary</h2>
```
9.  cart/templates/cart/checkout.html
```
...
        <div class="w-full md:pr-6"> //加了md:
            <div class="mb-6 p-6 bg-gray-100 rounded-xl">
                <h2 class="mb-5 uppercase text-lg">Shipping information</h2>
...
```
10. cart/templates/cart/partials/cart_item.html
```
...
    <a href="{% url 'product' item.product.slug %}" class="w-1/4"> //# 轉 {% url 'product' product.slug %}
        <img class="hover:shadow-lg rounded-xl" src="{{ item.product.get_thumbnail }}">
    </a>

    <div class="w-3/4 pl-6">
        <div class="flex justify-between">
            <a href="{% url 'product' item.product.slug %}" class="text-lg">{{ item.product.name }}</a> //# 轉 {% url 'product' product.slug %}
...

```
11. core/templates/core/frontpage.html
```
...
        <a href="{% url 'shop' %}" class="inline-block px-8 py-4 rounded-xl bg-white text-purple-500 hover:bg-gray-200">Start shopping</a> //# 轉成 {% url 'shop' %}
...
```
---
指南（二十）-- 3946
...
from django.contrib.auth.models import User
...
class Review(models.Model): //增加 class Review
...
---
## 新建一個 form 給 reviews用 
1. product/templates/product/product.html
```
...
  <a 
            href="#" 
    ...
    <div class="w-full mt-6">
        ...
        {% if request.user.is_authenticated %} //加入form
...
2. product/views.py
```
from django.shortcuts import render, get_object_or_404,redirect  //加了redirect

from .models import Product,Review //add Review models
...
...
...
        ...
        {% if product.get_rating > 0 %}  //加入rating
...
要做的事：
 ...
 ---
 我相信去到這裹應該出現了1-2個bug  
 1.singup頁面不能直接用link去 要打url去  
 2. 就是結算畫面的加減數量function不能正常運作  

 我希望你先試試能不能靠自己debug，如果不能就直接看下面吧
...
<div class="max-w-lg mx-auto flex flex-wrap p-6">
    <div class="w-full bg-gray-100 p-6 rounded-xl">
        ...
        <form method="post" action="." class="space-y-3">
            ...
            <div>
                <button class="py-4 px-6 rounded-xl text-white bg-purple-500 hover:bg-purple-700">提交</button>
            </div>
        </form>
      
    </div>  
    <p><a href="{% url 'signup' %}">沒有帳號，要注冊 請按這裹</a></p>  //加了這句
</div>
{% endblock %}
    ...
    if quantity:
        ...
        item = {
            'product': {
                ...
                'image': product.image,
                'get_thumbnail': product.get_thumbnail(),
                'slug': product.slug,  //增加 slug: product.slug
                'price': product.price,
            },
            'total_price': (quantity * product.price) / 100,
            'quantity': quantity,
